...
The mapping module utilizes Lidar scans and odometry data to generate a 2D occupancy grid of the environment. This dynamically constructed map allows the robot to identify free space and obstacles for navigation. Below is an example of a map generated by the physical robot:

![Map of real robot](maps/map_real_robot.png)
![Map of real robot](maps/mapping_fast_aliasing_drove_around.png)
...
The room was not fully mapped only as a proof of concept, due to too many objects that are not visible to the main sensor on the robot. This is the reason why the robot is only driving in a small part of the room and not the whole room. For example: chairs and cables on the ground were not recognized as obstacles, because they are too thin and small for the lidar sensor. Therefor, we decided on the proof of concept mapping above.
...
## Disclaimer
LLMs were used to help with formulation of the documentation, project report and code debugging and formatting.